...
const EditItemModal = ({ isOpen, onClose, onSave, item }) => {
  ...
  useEffect(() => {
    if (item) {
      ...
      setImagePreview(item.image || '/api/placeholder/200/150');
    }
  }, [item]);
  ...
  const handleImageUpload = async (e) => {
    ...
    if (file) {
      try {
        setUploading(true);
        const data = await uploadMenuImage(file); // Use API helper!
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xs sm:max-w-md max-h-[90vh] overflow-y-auto">
        <div className="p-4 sm:p-6">
          <div className="flex justify-between items-center mb-4 sm:mb-6">
            <div>
              <h2 className="text-lg sm:text-2xl font-bold text-slate-800">Edit Item</h2>
              <p className="text-xs sm:text-base text-slate-600">{item.name}</p>
            </div>
            <button
              onClick={handleClose}
